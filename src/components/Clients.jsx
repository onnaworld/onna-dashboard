            ...
            <div>
              ...
              <div style={{borderRadius:20,background:T.surface,border:`1px solid ${T.border}`,boxShadow:"0 2px 8px rgba(0,0,0,0.06)",padding:isMobile?20:32}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:16,borderTop:`1px solid ${T.borderSub}`}}>
                  <button onClick={async()=>{if(!confirm(`Delete ${selectedClient.company}?`))return;if(selectedClient._fromClient){archiveItem('clients',selectedClient);await api.delete(`/api/clients/${selectedClient.id}`);setLocalClients(prev=>prev.filter(x=>x.id!==selectedClient.id));}else if(selectedClient._fromLead&&selectedClient._fromOutreach){archiveItem('outreach',selectedClient);await api.delete(`/api/outreach/${selectedClient.id}`);setOutreach(prev=>prev.filter(x=>x.id!==selectedClient.id));}else if(selectedClient._fromLead){archiveItem('leads',selectedClient);await api.delete(`/api/leads/${selectedClient.id}`);setLocalLeads(prev=>prev.filter(x=>x.id!==selectedClient.id));}setSelectedClient(null);}} style={{background:"none",border:"none",color:"#c0392b",fontSize:12.5,fontWeight:500,cursor:"pointer",fontFamily:"inherit",padding:0}}>Delete client</button>
