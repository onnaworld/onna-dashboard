...
export function AddVendorModal({ T, isMobile, BtnPrimary, BtnSecondary, Sel, LocationPicker, CategoryPicker, showAddVendor, setShowAddVendor, newVendor, setNewVendor, api, setVendors, addNewOption, customVendorCats, setCustomVendorCats, allVendorCats, allLocations, customLocations, setCustomLocations, showAlert, doLogActivity }) {
  const [saving, setSaving] = useState(false);

  const handleSave = async () => {
    if (saving) return;
    if (!newVendor.name && !newVendor.company) { showAlert("Please enter a name or company."); return; }
    const payload = {};
    Object.keys(newVendor).forEach(k => { payload[k] = newVendor[k] || ""; });
    ...
    try {
      ...
      if (saved && saved.id) {
        ...
        if (doLogActivity) doLogActivity("created", "vendor", saved.id, saved.name || saved.company);
        setNewVendor({ name: "", company: "", category: "Locations", email: "", phone: "", website: "", location: "Dubai, UAE", notes: "", rateCard: "" });
        setShowAddVendor(false);
      } else if (saved && saved.queued) {
        // Offline — queued for sync
        const tempId = Date.now();
        setVendors(prev => [...prev, { ...payload, id: tempId, _queued: true }]);
        setNewVendor({ name: "", company: "", category: "Locations", email: "", phone: "", website: "", location: "Dubai, UAE", notes: "", rateCard: "" });
        setShowAddVendor(false);
      } else {
        showAlert("Failed to save vendor: " + (saved?.error || "Unknown error"));
      }
    } catch (e) {
      console.error("Vendor save error:", e);
      showAlert("Failed to save vendor: " + (e.message || "Network error"));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="modal-bg" onClick={() => setShowAddVendor(false)}>
      <div style={{ borderRadius: isMobile ? "20px 20px 0 0" : 20, padding: isMobile ? "24px 20px" : 28, width: isMobile ? "100%" : 520, maxWidth: isMobile ? "100%" : "92vw", background: T.surface, border: `1px solid ${T.border}`, boxShadow: "0 24px 60px rgba(0,0,0,0.15)", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
          ...
          <BtnPrimary onClick={handleSave} disabled={saving}>{saving ? "Saving…" : "Save Vendor"}</BtnPrimary>
        </div>
      </div>
    </div>
  );
