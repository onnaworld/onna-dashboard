...
function EstimateView({ estData, onSet: _rawOnSet, exchangeRate = 0.27, pendingReview, onAcceptMarker, onDeclineMarker, projectName }) {
  ...
  const doPrint = () => { const el=printRef.current; if(!el)return; const clone=el.cloneNode(true);
    ...
    // Replace inputs with their display values for clean print
    ...
    // A4 printable width at 96dpi ~= 720px (210mm - 20mm margins). Scale content to fit.
    const contentWidth = el.scrollWidth || 900;
    const a4Width = 720;
    const scale = contentWidth > a4Width ? a4Width / contentWidth : 1;
    clone.style.padding="40px 40px";clone.style.transformOrigin="top left";clone.style.transform=`scale(${scale})`;clone.style.width=`${contentWidth}px`;
