...
import { getToken, downloadCSV, rebalancePrintTableWidths } from "../../utils/helpers";
...
export default function Documents({
  ...
  if (documentsSubSection==="callsheet") {
        ...
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {csVersions.map((cs,i)=>{
              ...
              return(
                <div key={cs.id} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const d=e.dataTransfer.getData("text/plain");if(d.startsWith("cs:")){const from=+d.split(":")[1];if(from!==i){pushUndo("reorder call sheets");setCallSheetStore(prev=>{const store=JSON.parse(JSON.stringify(prev));const arr=store[p.id]||[];const[m]=arr.splice(from,1);arr.splice(i,0,m);store[p.id]=arr;return store;});}}}} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 20px",display:"flex",alignItems:"center",gap:14,cursor:"pointer",transition:"border-color 0.15s"}} onClick={()=>setActiveCSVersion(i)} onMouseEnter={e=>e.currentTarget.style.borderColor=T.accent} onMouseLeave={e=>e.currentTarget.style.borderColor=T.border}>
                  <span draggable onDragStart={e=>{e.stopPropagation();e.dataTransfer.setData("text/plain","cs:"+i);}} style={{color:T.muted,fontSize:14,cursor:"grab",userSelect:"none"}} onClick={e=>e.stopPropagation()}>☰</span>
                  <div style={{flex:1}}>
    ...
    const csSetTempC = (cKey, fKey, val) => csSet(d => { const n = parseFloat(val); return { ...d, [cKey]: val, [fKey]: val === "" ? "" : (isNaN(n) ? d[fKey] : String(Math.round(n * 9 / 5 + 32))) }; });
    const csSetTempF = (cKey, fKey, val) => csSet(d => { const n = parseFloat(val); return { ...d, [fKey]: val, [cKey]: val === "" ? "" : (isNaN(n) ? d[cKey] : String(Math.round((n - 32) * 5 / 9))) }; });
    ...
    const addAgentLine = (di,ci) => csSet(d => {
      ...
      delete cr.agent;
      const nextRole = existing.some(a=>a.role==="AGENT") ? (existing.some(a=>a.role==="PRODUCER")?"":"PRODUCER") : "AGENT";
    ...
    const setAgentField = (di,ci,ai,field,val) => csSet(d => {
      ...
      delete cr.agent;
      cr.agents[ai] = {...cr.agents[ai], [field]:val};
      return d;
    });
    ...
    // ── Reorderable call sheet sections ──
    const _sec_shoot = (<>
{/* SHOOT */}
...
{/* SCHEDULE */}
              ...
              <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed"}}>
                ...
                <tbody>
                  {csData.schedule.map((row,i) => (
                    <tr key={i} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const d=e.dataTransfer.getData("text/plain");if(d.startsWith("sched:")){const from=+d.split(":")[1];if(from!==i)csSet(dd=>{const a=[...dd.schedule];const[m]=a.splice(from,1);a.splice(i,0,m);return{...dd,schedule:a};});}}} style={{borderBottom:"1px solid #f0f0f0",background:CS_HL_BG[row.hl||""]||"#fff",breakInside:"avoid"}}>
                      <td data-noprint="1" style={{padding:"4px 2px",color:"#ccc",fontSize:10}}><span draggable onDragStart={e=>{e.dataTransfer.setData("text/plain","sched:"+i);}} style={{userSelect:"none",cursor:"grab"}}>☰</span></td>
                      <td style={{padding:"4px 4px 4px 0",fontSize:11,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:0}}>
                        <CSEditField value={row.time} onChange={v=>csU(`schedule.${i}.time`,v)} isPlaceholder autoFit placeholder="00:00" style={{fontSize:11,fontWeight:600}}/>
                      </td>
                      <td style={{padding:"4px 4px",fontSize:11,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:0}}>
                        <CSEditField value={row.activity} onChange={v=>csU(`schedule.${i}.activity`,v)} isPlaceholder autoFit placeholder="Activity" style={{fontSize:11,fontWeight:600}}/>
                      </td>
            ...
            {(csData.extraSchedules||[]).map((sched,si) => (
              <div key={si} draggable onDragStart={e=>{e.dataTransfer.setData("text/plain","xsched:"+si);e.currentTarget.style.opacity=0.4;}} onDragEnd={e=>{e.currentTarget.style.opacity=1;}} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const dt=e.dataTransfer.getData("text/plain");if(dt.startsWith("xsched:")){const from=+dt.split(":")[1];if(from!==si)moveSchedule(from,si);}}} style={{padding:"10px 32px"}}>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed"}}>
                  ...
                  <tbody>
                    {sched.rows.map((row,i) => (
                      <tr key={i} style={{borderBottom:"1px solid #f0f0f0",background:CS_HL_BG[row.hl||""]||"#fff",breakInside:"avoid"}}>
                        <td data-noprint="1" style={{padding:"4px 2px",color:"#ccc",fontSize:10}}><span style={{userSelect:"none"}}>☰</span></td>
                        <td style={{padding:"4px 4px 4px 0",fontSize:11,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:0}}>
                          <CSEditField value={row.time} onChange={v=>csU(`extraSchedules.${si}.rows.${i}.time`,v)} isPlaceholder autoFit placeholder="00:00" style={{fontSize:11,fontWeight:600}}/>
                        </td>
                        <td style={{padding:"4px 4px",fontSize:11,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:0}}>
                          <CSEditField value={row.activity} onChange={v=>csU(`extraSchedules.${si}.rows.${i}.activity`,v)} isPlaceholder autoFit placeholder="Activity" style={{fontSize:11,fontWeight:600}}/>
                        </td>
                        <td style={{padding:"4px 4px",fontSize:11,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:0}}>
                ...
                <CSAddBtn onClick={()=>addExtraScheduleRow(si)} label="Add Row"/>
              </div>
            ))}
            <div style={{padding:"0 32px 10px"}}>
              <CSAddBtn onClick={addSchedule} label="+ Add Schedule"/>
            </div>
    </>);
    const _sec_contacts = (<>
{/* CONTACTS */}
              ...
              <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed"}}>
                ...
                <tbody>
                  {csData.departments.map((dept,di) => { const deptOpen = !dept.discrete || !dept.collapsed; return (
                    <Fragment key={di}>
                      <tr onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const d=e.dataTransfer.getData("text/plain");if(d.startsWith("dept:")){const from=+d.split(":")[1];if(from!==di)csSet(dd=>{const a=[...dd.departments];const[m]=a.splice(from,1);a.splice(di,0,m);return{...dd,departments:a};});}}}><td colSpan={7} style={{padding:0}}>
                      ...
                      {deptOpen && dept.crew.map((cr,ci) => ([
                        <tr key={ci} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const d=e.dataTransfer.getData("text/plain");if(d.startsWith("crew:"+di+":")){const from=+d.split(":")[2];if(from!==ci)csSet(dd=>{const dept2={...dd.departments[di],crew:[...dd.departments[di].crew]};const[m]=dept2.crew.splice(from,1);dept2.crew.splice(ci,0,m);const depts=[...dd.departments];depts[di]=dept2;return{...dd,departments:depts};});}}} style={{background:CS_HL_BG[cr.hl||""]||"#fff",borderBottom:"1px solid #f5f5f5",breakInside:"avoid"}}>
                          ...
                          <td style={{padding:"3px 4px",fontSize:10,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:0}}>
                            <CSEditField value={cr.mobile} onChange={v=>csU(`departments.${di}.crew.${ci}.mobile`,v)} isPlaceholder autoFit style={{fontSize:10,fontStyle:dept.discrete?"italic":"normal",color:dept.discrete?"#777":undefined}} placeholder="Phone"/>
                          </td>
                          <td style={{padding:"3px 4px",fontSize:10,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:0}}>
                            <CSEditField value={cr.email} onChange={v=>csU(`departments.${di}.crew.${ci}.email`,v)} isPlaceholder autoFit style={{fontSize:10,color:dept.discrete?"#777":"#1565C0",fontStyle:dept.discrete?"italic":"normal"}} placeholder="Email"/>
                          </td>
                        ...
                        ...(cr.agents || (cr.agent ? [cr.agent] : [])).map((ag,ai) => (
                          <tr key={ci+"-agent-"+ai} style={{background:CS_HL_BG[ag.hl||""]||"#fafafa",borderBottom:"1px solid #f5f5f5"}}>
                            ...
                            <td style={{padding:"3px 4px",fontSize:10,color:"#999",fontStyle:"italic",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:0}}>
                              <CSEditField value={ag.name} onChange={v=>setAgentField(di,ci,ai,"name",v)} isPlaceholder style={{fontSize:10,color:"#999",fontStyle:"italic"}} placeholder="Name"/>
                            </td>
                            <td style={{padding:"3px 4px",fontSize:10,color:"#999",fontStyle:"italic",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:0}}>
                              <CSEditField value={ag.mobile} onChange={v=>setAgentField(di,ci,ai,"mobile",v)} isPlaceholder style={{fontSize:10,color:"#999",fontStyle:"italic"}} placeholder="Phone"/>
                            </td>
                            <td style={{padding:"3px 4px",fontSize:10,color:"#999",fontStyle:"italic",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:0}}>
                              <CSEditField value={ag.email} onChange={v=>setAgentField(di,ci,ai,"email",v)} isPlaceholder style={{fontSize:10,color:"#999",fontStyle:"italic"}} placeholder="Email"/>
                            </td>
...
{/* WEATHER */}
            <div style={{padding:"10px 32px 14px",background:CS_HL_BG[csData.weatherHl||""],borderRadius:3}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4,fontSize:10,fontFamily:CS_FONT}}>
                <div><span style={{fontWeight:700,letterSpacing:CS_LS,fontSize:9,color:"#888"}}>HIGH: </span><CSEditField value={csData.weatherHighC||""} onChange={v=>csSetTempC("weatherHighC","weatherHighF",v)} isPlaceholder style={{fontSize:10,minWidth:20}} placeholder="—"/>°C / <CSEditField value={csData.weatherHighF||""} onChange={v=>csSetTempF("weatherHighC","weatherHighF",v)} isPlaceholder style={{fontSize:10,minWidth:20}} placeholder="—"/>°F</div>
                <div><span style={{fontWeight:700,letterSpacing:CS_LS,fontSize:9,color:"#888"}}>LOW: </span><CSEditField value={csData.weatherLowC||""} onChange={v=>csSetTempC("weatherLowC","weatherLowF",v)} isPlaceholder style={{fontSize:10,minWidth:20}} placeholder="—"/>°C / <CSEditField value={csData.weatherLowF||""} onChange={v=>csSetTempF("weatherLowC","weatherLowF",v)} isPlaceholder style={{fontSize:10,minWidth:20}} placeholder="—"/>°F</div>
              </div>
              {csData.weatherShowRealFeel !== false ? (
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,fontSize:10,fontFamily:CS_FONT}}>
                  <div><span style={{fontWeight:700,letterSpacing:CS_LS,fontSize:9,color:"#888"}}>REAL FEEL HIGH: </span><CSEditField value={csData.weatherRealFeelHighC||""} onChange={v=>csSetTempC("weatherRealFeelHighC","weatherRealFeelHighF",v)} isPlaceholder style={{fontSize:10,minWidth:20}} placeholder="—"/>°C / <CSEditField value={csData.weatherRealFeelHighF||""} onChange={v=>csSetTempF("weatherRealFeelHighC","weatherRealFeelHighF",v)} isPlaceholder style={{fontSize:10,minWidth:20}} placeholder="—"/>°F</div>
                  <div><span style={{fontWeight:700,letterSpacing:CS_LS,fontSize:9,color:"#888"}}>REAL FEEL LOW: </span><CSEditField value={csData.weatherRealFeelLowC||""} onChange={v=>csSetTempC("weatherRealFeelLowC","weatherRealFeelLowF",v)} isPlaceholder style={{fontSize:10,minWidth:20}} placeholder="—"/>°C / <CSEditField value={csData.weatherRealFeelLowF||""} onChange={v=>csSetTempF("weatherRealFeelLowC","weatherRealFeelLowF",v)} isPlaceholder style={{fontSize:10,minWidth:20}} placeholder="—"/>°F</div>
                  <button data-noprint="1" onClick={()=>csU("weatherShowRealFeel",false)} style={{background:"none",border:"1px solid #eee",borderRadius:4,fontSize:8,color:"#bbb",cursor:"pointer",padding:"2px 6px",whiteSpace:"nowrap",fontFamily:"inherit",marginLeft:8}} onMouseEnter={e=>{e.currentTarget.style.color="#d32f2f";e.currentTarget.style.borderColor="#f5c6cb";}} onMouseLeave={e=>{e.currentTarget.style.color="#bbb";e.currentTarget.style.borderColor="#eee";}}>Remove</button>
                </div>
              ) : (
                <button data-noprint="1" onClick={()=>csU("weatherShowRealFeel",true)} style={{background:"none",border:"1px dashed #ddd",borderRadius:4,fontSize:9,color:"#999",cursor:"pointer",padding:"3px 10px",marginBottom:8,fontFamily:"inherit"}}>+ Add Real Feel</button>
              )}
              {csData.weatherImage ? (
...
{/* INVOICING */}
            <div style={{padding:"14px 32px"}}>
              ...
              <div style={{fontSize:11}}>
                ...
                {csData.invoicing.showTrn !== false ? (
                  <div style={{marginTop:4,display:"flex",alignItems:"center",gap:6}}>
                    <strong>TRN:</strong> <CSEditField value={csData.invoicing.trn} onChange={v=>csU("invoicing.trn",v)} style={{fontSize:11}}/>
                    <button data-noprint="1" onClick={()=>csU("invoicing.showTrn",false)} style={{background:"none",border:"1px solid #eee",borderRadius:4,fontSize:8,color:"#bbb",cursor:"pointer",padding:"2px 6px",whiteSpace:"nowrap",fontFamily:"inherit"}} onMouseEnter={e=>{e.currentTarget.style.color="#d32f2f";e.currentTarget.style.borderColor="#f5c6cb";}} onMouseLeave={e=>{e.currentTarget.style.color="#bbb";e.currentTarget.style.borderColor="#eee";}}>Remove</button>
                  </div>
                ) : (
                  <button data-noprint="1" onClick={()=>csU("invoicing.showTrn",true)} style={{background:"none",border:"1px dashed #ddd",borderRadius:4,fontSize:9,color:"#999",cursor:"pointer",padding:"3px 10px",marginTop:4,fontFamily:"inherit"}}>+ Add TRN</button>
                )}
              </div>
            </div>
    </>);
...
{csSecOrder.map((key,ki) => (
              <div key={key} data-cs-section="1" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const d=e.dataTransfer.getData("text/plain");if(d.startsWith("csSec:")){const fromKey=d.slice(6);if(fromKey!==key&&key!=="productionContacts")moveSectionTo(fromKey,key);}}} style={{position:"relative"}}>
                ...
                {CS_SEC_MAP[key]}
                        ...
                        {/* FOOTER */}
            {csData.footer?.show !== false ? (
              <div style={{borderTop:"2px solid #000",margin:"16px 32px 0",padding:"14px 0 20px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
                <div>
                ...
                <button onClick={()=>csSet(d=>({...d,footer:{...(d.footer||{}),show:false}}))} style={{background:"none",border:"1px solid #eee",borderRadius:4,fontSize:8,color:"#bbb",cursor:"pointer",padding:"2px 6px",whiteSpace:"nowrap",fontFamily:"inherit"}} onMouseEnter={e=>{e.currentTarget.style.color="#d32f2f";e.currentTarget.style.borderColor="#f5c6cb";}} onMouseLeave={e=>{e.currentTarget.style.color="#bbb";e.currentTarget.style.borderColor="#eee";}}>Remove Footer</button>
              </div>
            ) : (
              <div style={{margin:"16px 32px 0",padding:"6px 0",textAlign:"center"}}>
                <button onClick={()=>csSet(d=>({...d,footer:{...(d.footer||{}),show:true}}))} style={{background:"none",border:"1px dashed #ddd",borderRadius:4,fontSize:9,color:"#999",cursor:"pointer",padding:"4px 12px",fontFamily:"inherit"}}>+ Add Footer</button>
              </div>
            )}
          </div>
        </div>
  ...
  if (documentsSubSection==="dietaries") {
    ...
    const dietExportPDF = (elId,orient) => {
      ...
      const clone=el.cloneNode(true);clone.querySelectorAll("button").forEach(b=>b.remove());clone.querySelectorAll("input[type=file]").forEach(b=>b.remove());
      const iframe=document.createElement("iframe");iframe.style.cssText="position:fixed;top:0;left:0;width:100%;height:100%;border:none;z-index:-9999;opacity:0;";document.body.appendChild(iframe);
      const doc=iframe.contentDocument;doc.open();doc.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${dietData.label||"Dietary List"} | ${p.name}</title><style>*{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;color-adjust:exact!important;}body{background:#fff;font-family:'Avenir','Avenir Next','Nunito Sans',sans-serif;}@media print{@page{margin:12mm;size:A4 ${_ori};}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;color-adjust:exact!important;}}${PRINT_CLEANUP_CSS}</style></head><body></body></html>`);doc.close();
