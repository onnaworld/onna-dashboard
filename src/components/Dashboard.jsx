...
const CalendarWidget = memo(function CalendarWidget({
  ...
  setGcalToken, setGcalEvents, allProjectsMerged,
  activeProjects, buildPath, setActiveTab, setSelectedProject, setProjectSection, pushNav,
}) {
  ...
  const sc = { Proposal: "#f57f17", Confirmed: "#1565c0", Active: "#147d50", Archived: "#86868b" };
              ...
              {cells.map((date,i)=>{
                ...
                return (
                  <div key={key} onClick={()=>setCalDayView(date)} style={{minHeight:isMobile?44:66,borderRadius:7,background:isToday?T.accent+"15":"transparent",border:isToday?`1.5px solid ${T.accent}44`:`1px solid ${T.borderSub}`,padding:isMobile?"3px":"4px 5px",display:"flex",flexDirection:"column",gap:2,overflow:"hidden",minWidth:0,cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background=isToday?T.accent+"25":"#f5f5f7"} onMouseLeave={e=>e.currentTarget.style.background=isToday?T.accent+"15":"transparent"}>
                    ...
                    {!isMobile&&projBars.slice(0,2).map(p=>{const c2=sc[p.status]||"#86868b";return <div key={p.id} title={p.name} style={{fontSize:9,background:c2+"22",color:c2,borderRadius:3,padding:"1px 4px",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontWeight:600,lineHeight:1.5,minWidth:0,width:"100%",boxSizing:"border-box"}}>{p.name}</div>;})}
                    {isMobile?(dayEvs.length>0&&<div style={{display:"flex",gap:2,flexWrap:"wrap",marginTop:1}}>{dayEvs.slice(0,4).map((ev,ei)=>{const c=ev.colorId?(gcalEventColors[ev.colorId]?.background||GCAL_COLORS[ev.colorId]||T.accent):(ev.calendarColor||T.accent);return <span key={ei} style={{width:5,height:5,borderRadius:"50%",background:c,display:"inline-block"}}/>;})}{projBars.slice(0,2).map(p=>{const c3=sc[p.status]||"#86868b";return <span key={`p${p.id}`} style={{width:5,height:5,borderRadius:"50%",background:c3,display:"inline-block"}}/>;})}</div>):(
...
export default function Dashboard({
  ...
  const renderCalendarProjects = useCallback(() => (
    <CalendarWidget
      T={T} isMobile={isMobile} gcalToken={gcalToken} gcalEvents={gcalEvents}
      gcalLoading={gcalLoading} gcalEventColors={gcalEventColors}
      calMonth={calMonth} setCalMonth={setCalMonth} calDayView={calDayView} setCalDayView={setCalDayView}
      outlookEvents={outlookEvents} outlookLoading={outlookLoading} outlookError={outlookError}
      fetchOutlookCal={fetchOutlookCal} connectGCal={connectGCal}
      GCAL_CLIENT_ID={GCAL_CLIENT_ID} GCAL_COLORS={GCAL_COLORS}
      setGcalToken={setGcalToken} setGcalEvents={setGcalEvents}
      allProjectsMerged={allProjectsMerged} activeProjects={activeProjects}
      buildPath={buildPath} setActiveTab={setActiveTab} setSelectedProject={setSelectedProject}
      setProjectSection={setProjectSection} pushNav={pushNav}
    />
  ), [T,isMobile,gcalToken,gcalEvents,gcalLoading,gcalEventColors,calMonth,calDayView,outlookEvents,outlookLoading,outlookError,allProjectsMerged,activeProjects]);
  ...
  const renderTodos = useCallback(() => {
    ...
    const reorderDrop=(targetId,targetSubType,e)=>{
      ...
      if(e.dataTransfer.types.includes("text/list-drag"))return; // ignore list drags
    ...
    const colDrop=(day)=>({
      onDragOver:e=>{if(e.dataTransfer.types.includes("text/list-drag"))return;e.preventDefault();e.currentTarget.style.background=T.accent+"15";},
      onDragLeave:e=>{e.currentTarget.style.background="transparent";},
      onDrop:e=>{
        // Handle list reorder drops separately
        if(e.dataTransfer.types.includes("text/list-drag"))return;
        e.preventDefault();e.currentTarget.style.background="transparent";
        const dragId=Number(e.dataTransfer.getData("text/plain"));if(!dragId)return;
        pushUndo("move task");setTodos(prev=>prev.map(t=>t.id===dragId?{...t,tab:"onna",subType:day}:t));
      }
    });
        ...
        <div style={{padding:"6px 12px",overflowY:"auto",flex:1,minHeight:0}}>
          {todoTopFilter==="todo"?(<div>
            ...
            {customLists.map((cl,li)=>{
              ...
              return (
              <div key={cl} style={{marginTop:8,padding:"4px 0",borderTop:`1px solid ${T.borderSub}`}}
                onDragOver={e=>{
                  if(e.dataTransfer.types.includes("text/list-drag")){e.preventDefault();e.currentTarget.style.borderTopColor=T.accent;return;}
                  // task drop — allow
                  e.preventDefault();e.currentTarget.style.background=T.accent+"15";
                }}
                onDragLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.borderTopColor=T.borderSub;}}
                onDrop={e=>{
                  e.preventDefault();
                  e.currentTarget.style.background="transparent";
                  e.currentTarget.style.borderTopColor=T.borderSub;
                  // List reorder
                  if(e.dataTransfer.types.includes("text/list-drag")){
                    const fromIdx=Number(e.dataTransfer.getData("text/list-drag"));
                    if(fromIdx===li||isNaN(fromIdx))return;
                    const arr=[...customLists];const [item]=arr.splice(fromIdx,1);arr.splice(li,0,item);saveCustomLists(arr);setDragListIdx(null);
                    return;
                  }
                  // Task drop onto list
                  const dragId=Number(e.dataTransfer.getData("text/plain"));if(!dragId)return;
                  pushUndo("move task");setTodos(prev=>prev.map(t=>t.id===dragId?{...t,tab:"onna",subType:`list:${cl}`}:t));
                }}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,flex:1,minWidth:0}}>
                    ...
                    {editingListName===cl?(
                      ...
                      <div onDoubleClick={()=>setEditingListName(cl)} style={{fontSize:10,fontWeight:600,color:T.muted,textTransform:"uppercase",letterSpacing:"0.05em",cursor:"pointer",flex:1,minWidth:0}} title="Double-click to rename">{cl}</div>
                    )}
                  </div>
  ...
  ), [dashNotesList,dashSelectedNoteId,isMobile]);
