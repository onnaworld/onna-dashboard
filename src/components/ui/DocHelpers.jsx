...
const CSEditField = ({ value, onChange, style = {}, placeholder = "", bold = false, isPlaceholder = false, alwaysYellow = false, autoFit = false }) => {
  ...
  return <span ref={spanRef} data-yellow={showYellow?"1":undefined} onClick={()=>{setTemp(value);setEditing(true);}} style={{...style,fontFamily:CS_FONT,fontWeight:bold?700:style.fontWeight||400,cursor:"text",display:"inline-block",minWidth:16,minHeight:14,maxWidth:autoFit?undefined:"100%",boxSizing:"border-box",background:showYellow?"#FFFDE7":"transparent",borderRadius:showYellow?2:0,padding:showYellow?"1px 4px":0,borderBottom:"1px dashed transparent",transition:"all 0.15s",whiteSpace:autoFit?"nowrap":"pre-wrap",wordBreak:autoFit?"normal":"break-word",overflowWrap:autoFit?undefined:"anywhere"}} onMouseEnter={e=>(e.target.style.borderBottom="1px dashed #ccc")} onMouseLeave={e=>(e.target.style.borderBottom="1px dashed transparent")}>{value?renderCSLinks(value):<span data-noprint="1" style={{color:"#999",fontSize:style.fontSize||10}}>{placeholder}</span>}</span>;
};
...
const CS_IMG_MAX_W = 800; // approx available content width inside the 880px document
const CSResizableImage = ({ label, image, onUpload, onRemove, defaultHeight = 180 }) => {
  const ref = useRef();
  const [dims, setDims] = useState(null); // {w,h} once we know the image's aspect ratio
  ...
  const onMouseDown = useCallback(e => {
    e.preventDefault();
    const startH = (dims && dims.h) || defaultHeight;
    dragRef.current = { dragging:true, startY:e.clientY, startH };
    const onMove = ev => { if(!dragRef.current.dragging)return; const newH=Math.max(80,dragRef.current.startH+(ev.clientY-dragRef.current.startY)); setDims({ w: Math.round(newH*aspectRef.current), h: newH }); };
...
const CALLSHEET_INIT = {
  ...
  emergencyNumbers:[{label:"TBC",number:"TBC"}],
  emergencyDialPrefix:"TBC",
  emergency:{hospital:"TBC",police:"TBC"},
  invoicing:{crewLabel:"FOR CREW:",terms:"NET 30 days",email:"accounts@onnaproduction.com",address:"ONNA FILM, TV & RADIO PRODUCTION SERVICES LLC.\nOFFICE NO. F1-022,\nPROPERTY INVESTMENT OFFICE 4-F1\nDUBAI, UNITED ARAB EMIRATES",trn:"105161036600003"},
