    ...
    <div>
      ...
      <div style={{marginBottom:20,display:"flex",alignItems:"flex-start",gap:14}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0,marginTop:6}}>
          <select value={p.month||new Date().getMonth()+1} onChange={e=>{const v=Number(e.target.value);setLocalProjects(prev=>{const u=prev.map(pr=>pr.id===p.id?{...pr,month:v}:pr);try{localStorage.setItem('onna_cache_projects',JSON.stringify(u))}catch{}return u;});setSelectedProject(prev=>prev&&prev.id===p.id?{...prev,month:v}:prev);api.put(`/api/projects/${p.id}`,{month:v}).catch(()=>{});}} style={{padding:"5px 8px",borderRadius:8,fontSize:11,fontWeight:500,border:`1px solid ${T.border}`,fontFamily:"inherit",cursor:"pointer",color:T.text,background:T.surface,flexShrink:0}}>
            {["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"].map((m,i)=><option key={i+1} value={i+1}>{m}</option>)}
          </select>
          <select value={p.year||new Date().getFullYear()} onChange={e=>{const v=Number(e.target.value);setLocalProjects(prev=>{const u=prev.map(pr=>pr.id===p.id?{...pr,year:v}:pr);try{localStorage.setItem('onna_cache_projects',JSON.stringify(u))}catch{}return u;});setSelectedProject(prev=>prev&&prev.id===p.id?{...prev,year:v}:prev);api.put(`/api/projects/${p.id}`,{year:v}).catch(()=>{});}} style={{padding:"5px 8px",borderRadius:8,fontSize:11,fontWeight:500,border:`1px solid ${T.border}`,fontFamily:"inherit",cursor:"pointer",color:T.text,background:T.surface,flexShrink:0}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:isMobile?10:14,marginBottom:isMobile?16:28}}>
        {[[metricsMode==="finals"?"Invoiced":"Estimate Total",`AED ${totalIn.toLocaleString()}`,metricsMode==="finals"?"from invoices":"from estimate"],[metricsMode==="finals"?"Finals":"Actuals Total",`AED ${totalOut.toLocaleString()}`,metricsMode==="finals"?"reconciled costs":"tracked costs"],["Net Profit",`AED ${profit.toLocaleString()}`,metricsMode==="finals"?"invoiced − finals":"estimate − actuals"],["Margin",`${margin}%`,"net / revenue"]].map(([l,v,s])=>(
