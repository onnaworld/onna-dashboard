...
export const exportDocPreview=(preview,originalDoc,pageIndex)=>{
  ...
  const doExport=(orient)=>{
    ...
    idoc.open();idoc.write(`<!DOCTYPE html><html><head><style>*{margin:0;padding:0;box-sizing:border-box;}body{background:#fff;}@page{margin:0;size:${pw} ${ph};}html,body{width:100%;height:100%;}div{width:100%;height:100%;padding:10mm 12mm;display:flex;align-items:flex-start;justify-content:center;page-break-after:always;page-break-inside:avoid;overflow:hidden;}div:last-child{page-break-after:auto;}img{max-width:100%;max-height:100%;object-fit:contain;display:block;}</style></head><body>${pages.map(p=>`<div><img src="${p}"/></div>`).join("")}</body></html>`);idoc.close();
...
const _inFlight = {};
const _lastSaveAt = {};
export const hasPendingSave = (table, pid) => { const key = `${table}:${pid}`; return !!(_pendingFlush[key] || _saveTimers[key] || _inFlight[key]); };
// A hydration poll can be in flight (already asked the server for "the
// latest data") at the exact moment a save for the same document completes.
// By the time the poll's response arrives, hasPendingSave() correctly says
// "nothing pending" — but that response may have been generated by the
// server BEFORE it processed the save, so applying it would silently
// revert an edit that had already succeeded. Comparing against the save's
// completion timestamp (not just a pending/not-pending flag) closes that:
// if a save finished at or after the poll started, the poll's data can't
// be trusted regardless of its own pending state.
export const isSafeToHydrate = (table, pid, requestStartedAt) => {
  const key = `${table}:${pid}`;
  if (hasPendingSave(table, pid)) return false;
  const savedAt = _lastSaveAt[key];
  return !savedAt || savedAt < requestStartedAt;
};
...
export const debouncedGlobalSave = (table, data, delay = 500) => {
  ...
  _notifySaving();
  const fire = () => { _inFlight[table] = true; globalApi.put(table, data).then(() => _notifySaved(table)).catch(_notifySaveError).finally(() => { delete _inFlight[table]; }); };
  _pendingFlush[table] = fire;
...
export const debouncedConfigSave = (key, data, delay = 500) => {
  ...
  _notifySaving();
  const fire = () => { _inFlight[fk] = true; configApi.put(key, data).then(() => _notifySaved(fk)).catch(_notifySaveError).finally(() => { delete _inFlight[fk]; }); };
  _pendingFlush[fk] = fire;
...
export const FINANCE_SLUGS = ["stats","pnl","cashflow","arap","tax","expenses","pipeline","invoices"];
...
export const printCallSheetPDF = (cs) => {
  ...
  const venueHTML = (cs.venueRows||[]).map(r=>`<div style="display:flex;align-items:flex-start;margin-bottom:5px;gap:8px"><div style="width:100px;flex-shrink:0;font-size:9px;font-weight:700;color:#888;${LS}text-transform:uppercase">${e(r.label)}</div><div style="flex:1;min-width:0;font-size:11px;overflow-wrap:anywhere">${e(r.value)}</div></div>`).join("");
  ...
  const deptHTML = (cs.departments||[]).map(dept => {
    ...
    const crewRows = (discrete && dept.collapsed) ? "" : (dept.crew||[]).map(c=>{
      ...
      const agentRow = (c.agents || (c.agent ? [c.agent] : [])).map(ag=>`<tr style="background:#fafafa;border-bottom:1px solid #f5f5f5"><td style="padding:3px 4px;font-size:9px;color:#999;font-style:italic">${e(ag.role)}</td><td style="padding:3px 4px;font-size:10px;color:#999;font-style:italic">${e(ag.name)}</td><td style="padding:3px 4px;font-size:10px;color:#999;font-style:italic">${e(ag.mobile)}</td><td style="padding:3px 4px;font-size:10px;color:#999;font-style:italic">${e(ag.email)}</td><td style="padding:3px 8px 3px 4px;font-size:10px;font-weight:600;text-align:right;color:#999;font-style:italic">${e(ag.callTime)}</td></tr>`).join("");
      return row + agentRow;
    }).join("");
