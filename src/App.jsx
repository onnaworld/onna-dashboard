...
const OUTREACH_STATUSES = ["cold","warm","open"];
...
const SEED_LEADS = [
  {id:1,  company:"Apex Media",        contact:"Sarah Chen",    email:"sarah@apexmedia.com",       phone:"0412 345 678",     source:"Referral",     status:"Responded",            value:8500,  category:"Creative Agencies",    location:"Sydney",      followUp:"2026-03-01"},
  {id:2,  company:"Blue Horizon",      contact:"James Okafor", email:"james@bluehorizon.co",       phone:"0423 456 789",     source:"LinkedIn",     status:"New Lead",             value:12000, category:"Production Companies", location:"Melbourne",   followUp:"2026-03-05"},
  {id:3,  company:"Craft & Co",        contact:"Mia Torres",   email:"mia@craftco.com",            phone:"0434 567 890",     source:"Website",      status:"Meeting Arranged",     value:4200,  category:"Fashion",              location:"Brisbane",    followUp:"2026-02-28"},
  {id:4,  company:"Nova Tech",         contact:"Liam Park",    email:"liam@novatech.io",           phone:"0445 678 901",     source:"Cold Outreach",status:"Converted to Client",  value:22000, category:"Commercial",           location:"Sydney",      followUp:"2026-03-10"},
  {id:5,  company:"Sunrise Studios",   contact:"Priya Nair",   email:"priya@sunrisestudios.com",   phone:"0456 789 012",     source:"Referral",     status:"New Lead",             value:6700,  category:"Production Companies", location:"Perth",       followUp:"2026-03-03"},
  {id:9,  company:"Maison Lumière",    contact:"Céline Dupont",email:"celine@maisonlumiere.com",   phone:"+33 1 44 55 66",   source:"Referral",     status:"Responded",            value:28000, category:"Beauty & Fragrance",   location:"Dubai, UAE",  followUp:"2026-03-14"},
  {id:10, company:"Crescent Watch Co", contact:"Omar Al Farsi",email:"omar@crescentwatch.ae",      phone:"+971 50 123 4567", source:"LinkedIn",     status:"New Lead",             value:15000, category:"Jewellery & Watches",  location:"Dubai, UAE",  followUp:"2026-03-18"},
  {id:11, company:"Vogue Arabia",      contact:"Nadia Hassan", email:"nadia@voguearabia.com",      phone:"+971 4 123 4567",  source:"Referral",     status:"Meeting Arranged",     value:11000, category:"Editorial",            location:"Dubai, UAE",  followUp:"2026-03-20"},
];
const SEED_CLIENTS = [
...
const initOutreach = [
  {id:1, company:"Rimowa",      clientName:"Priya Singh",   role:"Head of Marketing", email:"priya@rimowa.com",  date:"2026-02-27", category:"Fashion",           location:"Dubai, UAE",  status:"cold", notes:"Initial intro email sent."},
  {id:2, company:"Chanel UAE",  clientName:"Leila Mansour", role:"Brand Manager",     email:"leila@chanel.ae",  date:"2026-02-20", category:"Beauty & Fragrance", location:"Dubai, UAE",  status:"warm", notes:"Responded — interested in Q3 campaign."},
  {id:3, company:"LVMH London", clientName:"James Holt",    role:"Creative Director", email:"jholt@lvmh.com",   date:"2026-01-15", category:"Jewellery & Watches",location:"London, UK",  status:"open", notes:"Meeting 10 March at their offices."},
];
...
const Badge = ({status}) => {
  const map = {"New Lead":["#f0f0f5",T.sub],"Responded":["#e8f0ff","#1a56db"],"Meeting Arranged":["#fff8e8","#92680a"],"Converted to Client":["#edfaf3","#147d50"]};
...
const OutreachBadge = ({status,onClick}) => {
  const s = {cold:{bg:"#f5f5f7",c:T.sub,label:"Cold"},warm:{bg:"#eef4ff",c:"#1a56db",label:"Warm"},open:{bg:"#edfaf3",c:"#147d50",label:"Open"}}[status]||{bg:"#f5f5f7",c:T.sub,label:"Cold"};
...
export default function OnnaDashboard() {
  ...
  const newCount      = localLeads.filter(l=>l.status==="New Lead").length;
  ...
  const outreachMonths     = ["All",...Array.from(new Set(outreach.map(o=>{const d=new Date(o.date);return `${MONTHS[d.getMonth()]} ${d.getFullYear()}`;  })))];
  const filteredOutreach   = outreach.filter(o=>{
    const q=getSearch("Outreach").toLowerCase();
    return (!q||o.company.toLowerCase().includes(q)||o.clientName.toLowerCase().includes(q))&&(outreachLocFilter==="All"||o.location===outreachLocFilter)&&(outreachCatFilter==="All"||o.category===outreachCatFilter)&&(outreachMonthFilter==="All"||`${MONTHS[new Date(o.date).getMonth()]} ${new Date(o.date).getFullYear()}`===outreachMonthFilter);
  });
        ...
        <div style={{flex:1,overflowY:"auto",padding:"28px 28px 44px"}}>
          ...
          {activeTab==="Sales"&&(
            <div>
              ...
              {leadsView==="dashboard"&&(()=>{
                const STATUSES = ["New Lead","Responded","Meeting Arranged","Converted to Client"];
                const COLORS   = {"New Lead":"#6e6e73","Responded":"#1a56db","Meeting Arranged":"#92680a","Converted to Client":"#147d50"};
                      ...
                      {STATUSES.map((s,i)=>(
                        <div key={s} style={{borderRadius:16,padding:"18px 20px",background:T.surface,border:`1px solid ${T.border}`,boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
                          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
                            ...
                            <span style={{fontSize:10,color:T.muted,letterSpacing:"0.05em",textTransform:"uppercase",fontWeight:600,lineHeight:1.3}}>{s}</span>
                          </div>
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:8}}>
                          {STATUSES.map((s,i)=>(counts[i]>0&&(
                            <div key={s} style={{display:"flex",alignItems:"center",gap:8}}>
                              ...
                              <span style={{fontSize:12.5,color:T.sub,flex:1}}>{s}</span>
              ...
              {leadsView==="leads"&&(
                <div>
                  ...
                  <div style={{borderRadius:16,overflow:"hidden",background:T.surface,border:`1px solid ${T.border}`,boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
                      <thead><tr><TH>Company</TH><TH>Contact</TH><TH>Category</TH><TH>Location</TH><TH>Source</TH><TH>Value</TH><TH>Status</TH><TH>Follow Up</TH></tr></thead>
                      <tbody>
                        {filteredLeads.map(l=>(
              ...
              {leadsView==="outreach"&&(
                <div>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16,flexWrap:"wrap"}}>
                    ...
                    <Sel value={outreachLocFilter} onChange={setOutreachLocFilter} options={outreachLocations} minWidth={155}/>
                    ...
                    <span style={{marginLeft:"auto",fontSize:12,color:T.muted}}>{filteredOutreach.length} contacts</span>
                  </div>
                  <div style={{display:"flex",gap:16,marginBottom:12}}>
                    {[["cold","No response",T.sub,"#f5f5f7"],["warm","Responded","#1a56db","#eef4ff"],["open","Meeting arranged","#147d50","#edfaf3"]].map(([s,l,c,bg])=>(
                      <div key={s} style={{display:"flex",alignItems:"center",gap:6,fontSize:11.5}}><span style={{width:7,height:7,borderRadius:"50%",background:bg,border:`1.5px solid ${c}`}}/><span style={{color:c,fontWeight:600,textTransform:"capitalize"}}>{s}</span><span style={{color:T.muted}}>— {l}</span></div>
                    ))}
                    <span style={{fontSize:11.5,color:T.muted,marginLeft:"auto"}}>Click badge to cycle</span>
                  </div>
                  <div style={{borderRadius:16,overflow:"hidden",background:T.surface,border:`1px solid ${T.border}`,boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
                      <thead><tr><TH>Company</TH><TH>Contact</TH><TH>Role</TH><TH>Email</TH><TH>Date</TH><TH>Category</TH><TH>Location</TH><TH>Status</TH><TH>Notes</TH><TH/></tr></thead>
                      <tbody>
        ...
        <div className="modal-bg" onClick={()=>setShowAddLead(false)}>
          <div style={{borderRadius:20,padding:28,width:520,maxWidth:"92vw",background:T.surface,border:`1px solid ${T.border}`,boxShadow:"0 24px 60px rgba(0,0,0,0.15)"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:18}}>
              ...
              <div>
                <div style={{fontSize:10,color:T.muted,marginBottom:5,letterSpacing:"0.06em",textTransform:"uppercase",fontWeight:500}}>Status</div>
                <Sel value={newLead.status} onChange={v=>setNewLead(p=>({...p,status:v}))} options={["New Lead","Responded","Meeting Arranged","Converted to Client"]} minWidth={200}/>
              </div>
            </div>
            <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
