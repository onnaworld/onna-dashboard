...
function OnnaDashboardInner() {
  ...
  useEffect(() => {
    setSaveStatusCallback((status, msg) => {
      ...
      else if (status === "error") { setSaveStatus("error"); setSaveErrorMsg(msg || ""); }
    });
  ...
  // Clean alias duplicates from customLocations on load
  useEffect(()=>{
    const cleaned = customLocations.filter(l=>!LOCATION_ALIASES[l]);
    if(cleaned.length!==customLocations.length) setCustomLocations(cleaned);
  },[]); // eslint-disable-line
    ...
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Text','Helvetica Neue',Arial,sans-serif",color:T.text,display:"flex"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto",overflowX:"auto",padding:`${P}px ${P}px 44px`}}>
          ...
          {/* ══ VENDORS ══ */}
          {activeTab==="Vendors"&&<Vendors T={T} isMobile={isMobile} api={api} bbCat={bbCat} setBbCat={setBbCat} bbLocation={bbLocation} setBbLocation={setBbLocation} filteredBB={filteredBB} customVendorCats={customVendorCats} setCustomVendorCats={setCustomVendorCats} customLocations={customLocations} setCustomLocations={setCustomLocations} allVendorCats={allVendorCats} allLocations={allLocations} addNewOption={addNewOption} getSearch={getSearch} setSearch={setSearch} setShowAddVendor={setShowAddVendor} setEditVendor={setEditVendor} getXContacts={getXContacts} vendors={vendors} setVendors={setVendors} archiveItem={archiveItem} pruneCustom={pruneCustom} downloadCSV={downloadCSV} exportTablePDF={exportTablePDF} SearchBar={SearchBar} Sel={Sel} TH={TH} TD={TD} BtnPrimary={BtnPrimary}/>}
        ...
        {/* ── INFORMATION TAB ── */}
        {activeTab==="Information"&&<Information T={T} api={api} isMobile={isMobile} notes={notes} setNotes={setNotes} notesLoading={notesLoading} setNotesLoading={setNotesLoading} archiveItem={archiveItem} BtnPrimary={BtnPrimary} BtnSecondary={BtnSecondary} hydrated={globalHydratedRef.current} templateFiles={templateFiles} setTemplateFiles={setTemplateFiles} tplProject={allProjectsMerged.find(p=>p.client==="TEMPLATE")} projectEstimates={projectEstimates} setProjectEstimates={setProjectEstimates} projectActuals={projectActuals} setProjectActuals={setProjectActuals} callSheetStore={callSheetStore} setCallSheetStore={setCallSheetStore} riskAssessmentStore={riskAssessmentStore} setRiskAssessmentStore={setRiskAssessmentStore} castingTableStore={castingTableStore} locDeckStore={locDeckStore} travelItineraryStore={travelItineraryStore} allProjects={allNonTemplateProjects} showAlert={showAlert} billieRateCards={billieRateCards} setBillieRateCards={setBillieRateCards}/>}
