// Vercel serverless function — Storyboard share link generator
...
async function findShareByToken(token, useAuth) {
  ...
  return list.find((e) => {
    if (e.type !== "storyboard_share") return false;
  ...
  try {
    ...
    if (req.method === "PUT") {
      ...
      const putResp = await fetch(`${BACKEND}/api/resources/${eid}`, {
        ...
        body: JSON.stringify({ type: "storyboard_share", blob: JSON.stringify(parsed) }),
      });
      if (!putResp.ok) return res.status(500).json({ error: "Failed to save feedback" });
      return res.status(200).json({ ok: true });
    }

    // POST — create or update Storyboard share
    if (req.method === "POST") {
      ...
      const slugBase = [clientSlug, projectSlug, "storyboard"].filter(Boolean).join("_") || "storyboard";
      const token = existingToken || (slugBase + "_v" + Date.now().toString(36));
      const url = `https://app.onna.digital/api/storyboard-share?token=${encodeURIComponent(token)}`;
      ...
      if (resourceId) {
        try {
          const putResp = await fetch(`${BACKEND}/api/resources/${resourceId}`, {
            method: "PUT",
            headers: backendHeaders(useAuth),
            body: JSON.stringify({ type: "storyboard_share", blob: blobData }),
          });
          if (putResp.ok) {
      ...
      if (existingToken) {
        try {
          ...
          if (listResp.ok) {
            ...
            const list = Array.isArray(entries) ? entries : entries.data || [];
            for (const e of list) {
              if (e.type !== "storyboard_share") continue;
              try {
      ...
      const payload = { type: "storyboard_share", blob: blobData };
    ...
    // GET — fetch Storyboard share by token
    if (req.method === "GET") {
      ...
      const title = parsed.projectName || "Storyboard";
...
.sb-wrap{max-width:1123px;margin:0 auto;background:#fff;border-radius:4px;box-shadow:0 2px 16px rgba(0,0,0,0.08);overflow:hidden}
.sb-inner{padding:24px 16px}
...
@media print{
  ...
  .sb-wrap{box-shadow:none;border-radius:0;max-width:100%;width:100%}
  .sb-inner{padding:8mm 10mm}
  .sb-inner img{max-width:100%;height:auto}
...
<div class="sb-wrap"><div class="sb-inner">${parsed.html}</div></div>
