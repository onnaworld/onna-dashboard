// Vercel serverless function — CPS share link generator
...
async function findShareByToken(token, useAuth) {
  ...
  return list.find((e) => {
    if (e.type !== "cps_share") return false;
  ...
  try {
    ...
    if (req.method === "PUT") {
      ...
      const putResp = await fetch(`${BACKEND}/api/resources/${eid}`, {
        ...
        body: JSON.stringify({ type: "cps_share", blob: JSON.stringify(parsed) }),
      });
    ...
    if (req.method === "POST") {
      ...
      const slugBase = [clientSlug, projectSlug, "cps"].filter(Boolean).join("_") || "cps";
      const token = existingToken || (slugBase + "_v" + Date.now().toString(36));
      const url = `https://app.onna.digital/api/cps-share?token=${encodeURIComponent(token)}`;
      ...
      if (resourceId) {
        try {
          const putResp = await fetch(`${BACKEND}/api/resources/${resourceId}`, {
            method: "PUT",
            headers: backendHeaders(useAuth),
            body: JSON.stringify({ type: "cps_share", blob: blobData }),
          });
          if (putResp.ok) {
      ...
      const payload = { type: "cps_share", blob: blobData };
    ...
    if (req.method === "GET") {
      ...
      const title = parsed.projectName || "Creative Production Schedule";
...
@media print{
  ...
  .cps-wrap{box-shadow:none;border-radius:0}
